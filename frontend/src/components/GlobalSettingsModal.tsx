...
export function GlobalSettingsModal({ onClose, notificationsEnabled, setNotificationsEnabled, themeAccent, setThemeAccent, myProfile, onProfileUpdated, showGameServers, setShowGameServers, shareSteamPresence, setShareSteamPresence }: GlobalSettingsModalProps) {
            ...
            <div className="bg-[#18181b] border border-white/10 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 md:p-6 overflow-y-auto space-y-6 flex-1">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/5">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg ${shareSteamPresence ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/10 text-white/50'}`}>
                                    <User size={20} />
                                </div>
                                <div>
                                    <div className="font-semibold text-white">Share Steam Presence</div>
                                    <div className="text-xs text-white/50">Show my current Steam game to others</div>
                                </div>
                            </div>
                            <button 
                                onClick={() => {
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${shareSteamPresence ? 'bg-emerald-500' : 'bg-white/20'}`}
                            >
